...
function buildOptions(header: ProgressivePlyHeader): ProgressivePlyBuildOptions {
  ...
  // The transfer cap is a hard safety property, even for unusual PLY schemas
...
export class ProgressivePlyService {
  ...
  async handleMessage(panel: vscode.WebviewPanel, message: any): Promise<boolean> {
    ...
    if (!acceptProgressiveGeneration(session, generation)) {
      return true;
    }
    ...
    for (const tileId of tileIds) {
      if (session.cancelled || !isCurrentProgressiveGeneration(session, generation)) break;
      try {
